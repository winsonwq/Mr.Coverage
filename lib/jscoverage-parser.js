...
_.extend(Parser.prototype, {
	init: function(jcData, whiteList) {
		...
		jsonObject.totalUnExecutedLineNum = 0;
		...
		_.each(this.source, function(jcObject, filename) {
			...
			var each = {
				filename: filename,
				source: jcObject.source,
				totalLines: jcObject,
				totalLineNum: jcObject.source.length,
				executedLineNum: filter(jcObject, function(num) {
					return num > 0;
				}).length,
				unExecutedLineNum : filter(jcObject, function(num) {
					return num === 0;
				}).length,
			};

			each.ignoreLineNum = each.totalLineNum - each.executedLineNum - each.unExecutedLineNum;
			...
			jsonObject.totalUnExecutedLineNum += each.unExecutedLineNum;
